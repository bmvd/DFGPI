...
namespace Valve.VR.InteractionSystem
{
    ...
    public class HoverButtonAsphodel : MonoBehaviour
    {
        ...
        public button_pressed buttonPressed;
        ...
        private void Start()
        {
            ...
            //ButtonEffectScript = GameObject.FindObjectOfType(typeof(ButtonEffect)) as ButtonEffect;
        }
        ...
        private void InvokeEvents(bool wasEngaged, bool isEngaged)
        {
            ...
            if (buttonDown && onButtonDown != null)
            {
                ...
                //ButtonEffectScript = keypad.GetComponent("ButtonPressed") as ButtonPressed;
                buttonPressed = FindObjectOfType<button_pressed>();
                string number = gameObject.name.Substring(gameObject.name.Length - 1);
                buttonPressed.OnPressNorm(number);
                //buttonDownNorm.Invoke(gameObject.name);
            }
            //onButtonDown(lastHoveredHand, gameObject.name);
            //Debug.Log(gameObject.name);
            //Debug.Log(lastHoveredHand);
